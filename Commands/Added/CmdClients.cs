    ...
    public class CmdClients : Command
    {
        ...
        public override void Use(Player p, string message)
        {
            try
            {
                ...
                foreach (Player pl in Player.players)
                {
                    if (!pl.hidden || p == null || p.group.Permission > LevelPermission.Operator)
                    {
                        if (String.IsNullOrEmpty(message) || !Group.Exists(message) || Group.Find(message) == pl.group)
                        {
                            ...
                            {
                                if (pl.voice)
                                    olddevs += " " + "&f+" + Server.DefaultColor + foundName + " (" + pl.level.name + "),";
                                else
                                    olddevs += " " + foundName + " (" + pl.level.name + "),";
                            }
                            if (pl.isMod) {
                                if (pl.voice)
                ...
                bool staff = devs.Length > 0 || olddevs.Length > 0 || mods.Length > 0 || gcmods.Length > 0;
                ...
                if (gcmods.Length > 0) {
                    Player.SendMessage(p, "#76MCForge GCMods:" + Server.DefaultColor + gcmods.Trim(','));
                }
