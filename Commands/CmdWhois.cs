    ...
    public class CmdWhois : Command
    {
        ...
        public override void Use(Player p, string message)
        {
            ...
            if (who != null && !who.hidden)
            {
                ...
                else if (who.isOldDev)
                {
                    Player.SendMessage(p, Server.DefaultColor + "> > Player is an original &9Developer");
                    if (Server.forgeProtection == ForgeProtection.Mod || Server.forgeProtection == ForgeProtection.oldDev)
                        Player.SendMessage(p, Server.DefaultColor + "> > Player is &CPROTECTED" + Server.DefaultColor + " under MCForge Staff protection");
                }
                else if (who.isMod)
                {
