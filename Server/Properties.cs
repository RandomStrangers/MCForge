...
if (!skipsalt)
{
    ...
    for (int i = 0; i < 16; ++i) { Server.salt += rndchars[rnd.Next(rndchars.Length)]; }
            ...
            if (!skipsalt)
            {
                ...
                if (!gotSalt2)
                {
                    ...
                    while (sb1.Length < 16)
                    {
                        ...
                        if (Char.IsLetterOrDigit((char)oneChar[0]))
                        {
                            sb1.Append((char)oneChar[0]);
                        }
                    }
                    Server.salt2 = sb1.ToString();
                }
                ...
                if (!skipsalt)
                {
                    ...
                    bool gotSalt3 = false;
                    bool gotSalt4 = false;
                    if (File.Exists("text/salt3.txt"))
                    {
                        string salt3 = File.ReadAllText("text/salt3.txt");
                        if (salt3.Length != 16)
                            Server.s.Log("Invalid salt in salt3.txt!");
                        else
                        {
                            Server.salt3 = salt3;
                            gotSalt3 = true;
                    ...
                    if (File.Exists("text/salt4.txt"))
                    {
                        string salt4 = File.ReadAllText("text/salt4.txt");
                        if (salt4.Length != 16)
                            Server.s.Log("Invalid salt in salt4.txt!");
                        else
                        {
                            Server.salt4 = salt4;
                            gotSalt4 = true;
                                    ...
                                    case "mcforge-protection-level":
                                        switch (value.ToLower())
                                        {
                                            case "dev":
                                                ...
                                                break;
                                        case "olddev":
                                            Server.forgeProtection = ForgeProtection.oldDev;
                                            break;
                                        case "mod":
		...
		public static void SaveProps(StreamWriter w) {
            ...
            w.WriteLine("server-owner = " + Server.Owner.ToString());
